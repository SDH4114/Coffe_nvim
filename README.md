...
- Стартовая страница **Coffe**: создать / открыть проект, последние проекты, файлы,
  поиск текста, настройки, справка. Появляется только при пустом запуске; файлы и stdin
  не заменяет. Вызвать вручную: `:Coffe`.
...
- **Telescope**: поиск файлов и текста; без него работает встроенный фильтр результатов.
...
| Поиск файлов / текста | `Space ff` / `Space fg` |
...
В Neo-tree нажми `?` для справки. Встроенная панель: `Enter` открыть / раскрыть,
`a` создать (путь с `/` в конце — папка), `r` переименовать, `D` удалить с подтверждением,
`.` скрытые файлы, `R` обновить, `q` закрыть. Удаляет только файлы и пустые папки.
Загруженные файлы перед переименованием или удалением нужно закрыть через `:bd`.
Во встроенном поиске вводи фильтр, выбирай стрелками, открывай Enter; Esc закрывает,
Ctrl-F позволяет ввести Unicode-фильтр.
